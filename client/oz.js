...
OZ.getAuthorizationHeader = function (method, uri, ticket, ext) {
    ...
    ext = (typeof ext === 'object' ? JSON.stringify(ext) : ext);
    ...
    var mac = OZ.macMessage(normalized, ticket);
    return 'Oz id="' + ticket.id + '", app="' + ticket.app + '", ts="' + Date.now() + (ext ? '", ext="' + ext : '') + '", mac="' + mac + '"';
};
