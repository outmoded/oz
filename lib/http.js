...
exports.authenticate = function (req, options, callback) {
    ...
    Ticket.parse(attributes.ticket, function (err, ticket) {
        ...
        if (!Request.validate(request, attributes, ticket)) {
            return (new Error('Invalid request MAC'));
        }

        // Check timestamp

        if (Math.abs(now - ticket.offset - attributes.ts) >= Settings.ticket.timestampWindow) {
            return (new Error('Request includes stale timestamp'));
        }
...
exports.formatHeader = function (header) {

    // Construct header

    return 'OZ ticket="' + header.ticket + '", ts="' + header.ts + (header.ext ? '", ext="' + header.ext : '') + '", mac="' + header.mac + '"';
};
