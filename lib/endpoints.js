...
exports.app = function (req, payload, options, callback) {
    ...
    options.loadAppFunc(creds.username, function (app) {
        ...
        if (!app) {
            return callback(Err.unauthorized('Invalid application identifier or secret', 'Basic'));
        }
...
exports.reissue = function (req, payload, options, callback) {
    ...
    var load = function (ticket) {
        ...
        options.loadAppFunc(ticket.app, function (app) {
            ...
            if (!app) {
                return callback(Err.unauthorized('Invalid application'));
            }
            ...
            options.loadGrantFunc(ticket.grant, function (grant, ext) {
                ...
                if (!grant ||
                    ...
                    return callback(Err.unauthorized('Invalid grant'));
                }
        ...
        internals.authenticate(req, 'app', options, function (err, ticket) {
            ...
            Rsvp.parse(payload.rsvp, options.encryptionPassword, function (err, envelope) {
                ...
                if (envelope.app !== ticket.app) {
                    return callback(Err.forbidden('Mismatching ticket and rsvp apps'));
                }

                var now = Date.now();

                if (envelope.exp <= now) {
                    return callback(Err.forbidden('Expired rsvp'));
                }
                ...
                options.loadGrantFunc(envelope.grant, function (grant, ext) {
                    ...
                    if (!grant ||
                        ...
                        return callback(Err.forbidden('Invalid grant'));
                    }

                    options.loadAppFunc(grant.app, function (app) {

                        if (!app) {
                            return callback(Err.forbidden('Invalid application identifier or secret'));
                        }
...
internals.authenticate = function (req, entity, options, callback) {
    ...
    Request.authenticate(req, options.encryptionPassword, { isHttps: options.isHttps }, function (err, ticket, ext) {
        ...
        if (entity === 'user') {
            if (!ticket.user) {
                return callback(Err.unauthorized('Application ticket cannot be used on a user endpoint'));
            }
        ...
        if (entity === 'app') {
            if (ticket.user) {
                return callback(Err.unauthorized('User ticket cannot be used on an application endpoint'));
            }
        ...
        return callback(Err.internal('Unknown endpoint entity mode'));
    });
};
...
internals.basicAuth = function (req) {
    ...
    if (!authorization) {
        return Err.unauthorized('Request missing authentication', 'Basic');
    }

    var parts = authorization.split(/\s+/);
    if (parts.length !== 2) {
        return Err.unauthorized('Bad HTTP authentication header format: ' + authorization, 'Basic');
    }

    if (parts[0].toLowerCase() !== 'basic') {
        return Err.unauthorized('Incorrect HTTP authentication scheme: ' + parts[0], 'Basic');
    }
