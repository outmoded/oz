...
exports.parse = function (req, options) {
    ...
    if (!req.headers.authorization) {
        return new Error('Request missing authorization header');
    }
    ...
    if (!attributes.id ||
        ...
        return new Error('Missing authentication attributes in Authorization header field');
    }
...
exports.parseAuthHeader = function (header) {
    ...
    if (verify) {                               // verify will be empty on full match
        ...
        return (new Error('Authorization header field includes unknown attributes: ' + verify));
    }
...
exports.authenticate = function (req, encryptionPassword, options, callback) {
    ...
    if (!encryptionPassword) {
        return internals.setErrorHeader(new Error('Internal error'), 'Invalid encryption password', callback);
    }
    ...
    if (request instanceof Error) {
        return internals.setErrorHeader(request, callback);
    }
    ...
    Ticket.parse(request.auth.id, encryptionPassword, function (err, ticket) {
        ...
        if (err) {
            return internals.setErrorHeader(err, callback);
        }
        ...
        if (ticket.delegatedBy &&
            ...
            return internals.setErrorHeader(new Error('Mismatching delegated application id'), callback);
        }

        if (ticket.app !== request.auth.app) {
            return internals.setErrorHeader(new Error('Mismatching application id'), callback);
        }

        // Validate MAC

        if (!exports.validateMac(request, ticket, request.auth)) {
            return internals.setErrorHeader(new Error('Invalid request MAC'), callback);
        }

        // Check timestamp

        if (Math.abs(now - ticket.offset - request.auth.ts) >= Settings.ticket.timestampWindow) {
            return internals.setErrorHeader(new Error('Request includes stale timestamp'), callback);
        }
...
internals.setErrorHeader = function (err /*, [message], callback */) {

    var message = (arguments.length === 3 ? arguments[1] : '');
    var callback = (arguments.length === 3 ? arguments[2] : arguments[1]);

    err.wwwAuthenticateHeader = 'Oz' + (message ? ' error="' + message + '"' : '');
    callback(err);
};
...
exports.formatHeader = function (attributes, id, app, mac) {
    ...
    return 'Oz id="' + id + '", app="' + exports.escapeHeaderAttribute(app) + '", ts="' + attributes.ts + (attributes.ext ? '", ext="' + exports.escapeHeaderAttribute(attributes.ext) : '') + (attributes.dlg ? '", dlg="' + exports.escapeHeaderAttribute(attributes.dlg) : '') + '", mac="' + mac + '"';
};
