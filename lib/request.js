...
exports.parse = function (req, options) {
    ...
    if (!hostHeader) {
        return Err.badRequest('Missing Host header field');
    }
    ...
    if (!hostParts ||
        ...
        return Err.badRequest('Invalid Host header field');
    }
    ...
    if (!attributes.id ||
        ...
        return Err.badRequest('Missing authentication attributes in Authorization header field');
    }
...
exports.parseAuthHeader = function (header) {
    ...
    if (!headerParts ||
        ...
        return (Err.internal('Wrong authentication scheme'));
    }
    ...
    if (verify) {                               // verify will be empty on full match
        ...
        return (Err.badRequest('Authorization header field includes unknown attributes: ' + verify));
    }
...
exports.authenticate = function (req, encryptionPassword, options, callback) {
    ...
    if (!encryptionPassword) {
        return callback(Err.internal('Invalid encryption password'));
    }
    ...
    Ticket.parse(request.auth.id, encryptionPassword, function (err, ticket) {
        ...
        if (ticket.exp <= now) {
            return callback(Err.unauthorized('Expired ticket'));
        }
        ...
        if (ticket.delegatedBy &&
            ...
            return callback(Err.unauthorized('Mismatching delegated application id'));
        }

        if (ticket.app !== request.auth.app) {
            return callback(Err.unauthorized('Mismatching application id'));
        }

        // Validate MAC

        if (!exports.validateMac(request, ticket, request.auth)) {
            return callback(Err.unauthorized('Invalid request MAC'));
        }

        // Check timestamp

        if (Math.abs(now - ticket.offset - request.auth.ts) >= Settings.ticket.timestampWindow) {
            return callback(Err.unauthorized('Request includes stale timestamp'));
        }
