...
exports.issue = function (app, grant, encryptionPassword, options, callback) {
    ...
    Utils.toss(app && app.id, Err.internal('Invalid application object'), callback);
    Utils.toss(!grant || (grant.id && grant.user && grant.exp), Err.internal('Invalid grant object'), callback);
    Utils.toss(encryptionPassword, Err.internal('Invalid encryption password'), callback);
    Utils.toss(options, Err.internal('Invalid options object'), callback);
    Utils.toss(!options.issueTo || options.issueTo !== app.id, Err.badRequest('Cannot issue to self'), callback);
...
exports.reissue = function (ticket, encryptionPassword, options, callback) {
    ...
    Utils.toss(ticket, Err.internal('Invalid original ticket object'), callback);
    Utils.toss(encryptionPassword, Err.internal('Invalid encryption password'), callback);
    Utils.toss(options, Err.internal('Invalid options object'), callback);
    Utils.toss(!options.scope || Scope.isSubset(ticket.scope, options.scope), Err.forbidden('New scope is not a subset of ticket scope'), callback);
    Utils.toss(!options.issueTo || !ticket.delegatedBy, Err.badRequest('Cannot re-delegate'), callback);
