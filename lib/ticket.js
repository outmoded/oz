...
exports.reissue = function (ticket, encryptionPassword, options, callback) {
    ...
    Hoek.toss(ticket, Boom.internal('Invalid original ticket object'), callback);
    Hoek.toss(encryptionPassword, Boom.internal('Invalid encryption password'), callback);
    Hoek.toss(options, Boom.internal('Invalid options object'), callback);
    Hoek.toss(!options.scope || Scope.isSubset(ticket.scope, options.scope), Boom.forbidden('New scope is not a subset of ticket scope'), callback);
    Hoek.toss(!options.issueTo || !ticket.delegatedBy, Boom.badRequest('Cannot re-delegate'), callback);
