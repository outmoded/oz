...
exports.issue = function (app, grant, encryptionPassword, options, callback) {
    ...
    Utils.toss(!grant || (grant.id && grant.user && grant.exp), 'Invalid grant object', callback);
    Utils.toss(encryptionPassword, 'Invalid encryption password', callback);
    Utils.toss(options, 'Invalid options object', callback);
    Utils.toss(!options.issueTo || options.issueTo !== app.id, 'Cannot issue to self', callback);
...
exports.reissue = function (ticket, encryptionPassword, options, callback) {
    ...
    Utils.toss(ticket, 'Invalid original ticket object', callback);
    Utils.toss(encryptionPassword, 'Invalid encryption password', callback);
    Utils.toss(options, 'Invalid options object', callback);
    Utils.toss(!options.scope || Scope.isSubset(ticket.scope, options.scope), 'New scope is not a subset of ticket scope', callback);
    Utils.toss(!options.issueTo || !ticket.delegatedBy, 'Cannot re-delegate', callback);
