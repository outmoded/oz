// Load modules

var Utils = require('./utils');
var Err = require('./error');


// Declare internals

var internals = {};


// Ensure scope is an array of unique strings

exports.validate = function (scope) {

    if (!scope) {
        return Err.internal('null scope');
    }

    if (scope instanceof Array === false) {
        return Err.internal('scope not instance of Array');
    }

    var hash = {};
    for (var i = 0, il = scope.length; i < il; ++i) {
        if (!scope[i]) {
            return Err.badRequest('scope includes null or empty string value');
        }

        if (typeof scope[i] !== 'string') {
            return Err.badRequest('scope item is not a string');
        }

        if (hash[scope[i]]) {
            return Err.badRequest('scope includes duplicated item');
        }

        hash[scope[i]] = true;
    }

    return true;
};


// Check is one scope is a subset of another

exports.isSubset = function (scope, subset) {

    if (scope.length < subset.length) {
        return false;
    }

    var common = Utils.intersect(scope, subset);
    return common.length === subset.length;
};